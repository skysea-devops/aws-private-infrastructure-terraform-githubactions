...
### 1. Docker Compose Dosyalarını Ekle

**docker/docker-compose.yml** (Production - Port 5678)
```bash
# Repository root'ta
mkdir -p docker
# docker-compose.yml dosyasını buraya kopyala
```

**docker/docker-compose.pr.yml** (PR Canary Override - Port 5679)
```bash
# docker-compose.pr.yml dosyasını docker/ klasörüne kopyala
```

---

### 2. GitHub Workflows Ekle

**.github/workflows/n8n-pr-canary.yml**
```bash
mkdir -p .github/workflows
# n8n-pr-canary.yml dosyasını buraya kopyala
```

**.github/workflows/n8n-deploy-prod.yml**
```bash
# n8n-deploy-prod.yml dosyasını buraya kopyala
```
...
# n8n Configuration
N8N_HOST = n8n.bensonpaca.com
N8N_ENCRYPTION_KEY = <generate-random-string>

# Database (from Terraform outputs or Secrets Manager)
DB_POSTGRESDB_HOST = myproject-dev-rds.xxxxx.us-east-1.rds.amazonaws.com
...
aws secretsmanager get-secret-value \
  --secret-id myproject/dev/rds-credentials \
  --query SecretString \
  --output text | jq .

# Generate Encryption Key (random 32 chars)
openssl rand -hex 16
```

---

### 4. Branch Protection Ekle (CTO için)
...
- ✅ Require status checks to pass before merging
  - **Required checks:**
    - `PR Canary Deployment Test` ✅

**Sonuç:** PR canary test fail ise merge **engellenir**!

---

### 5. Production Environment Ekle (Optional)
...
**Protection rules:**
- ✅ Required reviewers: CTO kullanıcı adı
- ⏱️ Wait timer: 0 minutes (veya 5 dk)

Bu sayede production deploy için approval gerekir.
