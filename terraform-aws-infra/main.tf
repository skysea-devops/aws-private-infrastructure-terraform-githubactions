...
  # Required for OIDC — keeps session on same instance
  stickiness {
    type            = "lb_cookie"
...
# HTTPS listener — default 403 (blocks unknown hosts)
...
  # Step 2: forward to target group
  action {
    type             = "forward"
    order            = 2
...
############################################
# ACM Certificate 
# the CNAME record in Route 53
############################################

resource "aws_acm_certificate" "service-cert" {
...
resource "aws_acm_certificate_validation" "service-cert" {
  certificate_arn = aws_acm_certificate.service-cert.arn
...
        Effect    = "Allow"
        Principal = { AWS = "arniam::${data.aws_elb_service_account.main.id}:root" }
        Action    = "s3:PutObject"
...
############################################
# VPC Flow Logs -> CloudWatch Logs — Phase 2
############################################
